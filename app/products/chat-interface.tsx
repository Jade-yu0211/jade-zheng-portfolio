...
const initialMessages: ChatMessage[] = [
  {
    ...
    role: "assistant",
    content:
      "你好，我是 Fitness Chat。你可以向我询问动作、营养与体态等训练问题。",
    isIntro: true,
  },
];

const suggestions = ["深蹲膝盖内扣怎么调整", "增肌期怎么安排蛋白质", "圆肩体态如何改善"];
...
export default function ChatInterface() {
  ...
  return (
    <div className="chat-window" aria-label="Fitness Chat 聊天窗口">
      <header className="chat-window-header">
        <div className="chat-window-identity">
          <span className="chat-window-avatar" aria-hidden="true">F</span>
          <span>
            <strong>Fitness Chat</strong>
          </span>
        </div>
        ...
        {messages.map((message) => (
          ...
          >
            {message.role === "assistant" ? (
              <span className="chat-message-avatar" aria-hidden="true">F</span>
            ) : null}
            <p>{message.content}</p>
          </div>
        ))}
